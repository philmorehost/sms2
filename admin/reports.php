...
// Get total number of message batches for all users
$total_records = $conn->query("SELECT COUNT(id) as total FROM messages")->fetch_assoc()['total'];
$total_pages = ceil($total_records / $limit);

// Fetch message batches for the current page
$messages = [];
$message_ids = [];
$stmt = $conn->prepare("SELECT m.*, u.username FROM messages m JOIN users u ON m.user_id = u.id ORDER BY m.created_at DESC LIMIT ? OFFSET ?");
