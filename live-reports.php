...
$base_sql = "FROM message_recipients mr JOIN messages m ON mr.message_id = m.id WHERE m.user_id = ?";
$where_clauses = [];
$params = [$current_user['id']];
$types = 'i';

if (!empty($search_term)) {
    $where_clauses[] = "(m.sender_id LIKE ? OR mr.recipient_number LIKE ?)";
...
$data_sql = "SELECT mr.recipient_number, mr.status, mr.failure_reason, mr.updated_at, m.message, m.sender_id, m.status as message_status, m.api_response " . $base_sql . " ORDER BY mr.updated_at DESC LIMIT ? OFFSET ?";
        ...
        <div class="card-tools">
            <form action="" method="GET" class="form-inline">
                <div class="input-group input-group-sm">
                    <input type="text" name="search" class="form-control" placeholder="Search Sender, Recipient..." value="<?php echo htmlspecialchars($_GET['search'] ?? ''); ?>">
