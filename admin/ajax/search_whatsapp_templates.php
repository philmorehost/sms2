...
$count_sql = "SELECT COUNT(t.id) as total FROM whatsapp_templates t JOIN users u ON t.user_id = u.id";
...
if (empty($submissions)) {
    $html = '<tr><td colspan="6" class="text-center">No WhatsApp Template submissions found.</td></tr>';
} else {
    foreach ($submissions as $sub) {
        $html .= '<tr>';
        $html .= '<td>' . htmlspecialchars($sub['username']) . '</td>';
        $html .= '<td>' . htmlspecialchars($sub['template_name']) . '</td>';
        $html .= '<td>' . nl2br(htmlspecialchars($sub['template_body'])) . '</td>';
        $html .= '<td>' . htmlspecialchars($sub['parameters_list']) . '</td>';
